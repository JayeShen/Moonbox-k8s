...
typeset SANDBOX_INSTALL_PREFIX="${HOME}"
...
function main() {
    ...
    cat ./bin/sandbox_win.sh \
        | sed "s:typeset SANDBOX_HOME_DIR:typeset SANDBOX_HOME_DIR=${SANDBOX_INSTALL_LOCAL}:g" \
        > ${SANDBOX_INSTALL_LOCAL}/bin/sandbox_win.sh \
        && chmod +x ${SANDBOX_INSTALL_LOCAL}/bin/sandbox_win.sh \
        || exit_on_err 1 "permission denied, replace ${SANDBOX_INSTALL_LOCAL}/bin/sandbox_win.sh failed."
