...
# this script run on windows git bash.
# please DO NOT put jdk in the "Program Files" directory, otherwise it could not run due to blank exists in the jdk path.
...
check_permission() {
  ...
  # check PID existed
  jps | grep "${TARGET_JVM_PID}" > /dev/null ||
    exit_on_err 1 "permission denied, java process ${TARGET_JVM_PID} is not existed."

  # check attach
  #grep -U "${SANDBOX_USER}" | grep "${TARGET_JVM_PID}" > /dev/null ||
...
reset_for_env() {
  ...
  SANDBOX_JAVA_HOME=$(echo $SANDBOX_JAVA_HOME | sed  's/\\/\//g' | sed 's/C:/\/c/g') 
...
function attach_jvm() {
  ...
  echo 'sandbox home:'$SANDBOX_JAVA_HOME
  # attach target jvm
  echo 'lib:'${SANDBOX_LIB_DIR}/sandbox-core.jar
  echo "ops:"${SANDBOX_JVM_OPS}
  echo 'target pid:'${TARGET_JVM_PID}
  echo "home=${SANDBOX_HOME_DIR};token=${token};server.ip=${TARGET_SERVER_IP};server.port=${TARGET_SERVER_PORT};namespace=${TARGET_NAMESPACE};task.config=${TASK_CONFIG}"
  "${SANDBOX_JAVA_HOME}/bin/java.exe" \
    ${SANDBOX_JVM_OPS} \
...
function main() {
  source /etc/profile
  while getopts "hp:vFfRu:a:A:d:m:I:P:ClSn:X:T:" ARG; do
    case ${ARG} in
    h)
      usage
      exit
