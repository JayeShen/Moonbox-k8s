...
[[ -z ${SANDBOX_HOME_DIR} ]] && SANDBOX_HOME_DIR=${HOME}/sandbox
...
# define target TASK_CONFIG
typeset  TASK_CONFIG

# define target CURRENT_JAVA_HOME
typeset  CURRENT_JAVA_HOME
...
reset_for_env() {
  echo "before set, SANDBOX_JAVA_HOME:"${SANDBOX_JAVA_HOME}
  # use the env JAVA_HOME for default
  echo "JAVA_HOME:"${JAVA_HOME}
  [[ -n "${JAVA_HOME}" ]] &&
    SANDBOX_JAVA_HOME="${JAVA_HOME}"
  echo "after set from JAVA_HOME, SANDBOX_JAVA_HOME:"${SANDBOX_JAVA_HOME}

  # use pass args
  [[ -z "${SANDBOX_JAVA_HOME}" ]] && SANDBOX_JAVA_HOME="${CURRENT_JAVA_HOME}"
  echo "after use pass args, SANDBOX_JAVA_HOME:"${SANDBOX_JAVA_HOME}
  ...
  echo "after set from TARGET_JVM, SANDBOX_JAVA_HOME:"${SANDBOX_JAVA_HOME}

  # append toos.jar to JVM_OPT
  if [[ -f "${SANDBOX_JAVA_HOME}"/lib/tools.jar ]]; then
    TOOLS_JAR="${SANDBOX_JAVA_HOME}/lib/tools.jar"
  else
    JDKHOME=$(echo ${SANDBOX_JAVA_HOME} | sed 's/\/jre//')
    TOOLS_JAR="${JDKHOME}/lib/tools.jar"
  fi
  echo "TOOLS_JAR:${TOOLS_JAR}"

  [[ -f "${TOOLS_JAR}" ]] &&
    SANDBOX_JVM_OPS="${SANDBOX_JVM_OPS} -Xbootclasspath/a:${TOOLS_JAR}"
...
function attach_jvm() {
  ...
  echo server_port is ${TARGET_SERVER_PORT}
  echo task_config is ${TASK_CONFIG}
  echo current_java_home is ${CURRENT_JAVA_HOME}
  echo snadbox_java_home is ${SANDBOX_JAVA_HOME}
  ...
  "${SANDBOX_JAVA_HOME}/bin/java" \
    ...
    "home=${SANDBOX_HOME_DIR};token=${token};server.ip=${TARGET_SERVER_IP};server.port=${TARGET_SERVER_PORT};namespace=${TARGET_NAMESPACE};task.config=${TASK_CONFIG}" ||
...
function main() {
  source /etc/profile
  while getopts "hp:vFfRu:a:A:d:m:I:P:ClSn:X:T:K:" ARG; do
    case ${ARG} in
    h)
      usage
      exit
    ...
    T) TASK_CONFIG=${OPTARG} ;;
    K) CURRENT_JAVA_HOME=${OPTARG} ;;
    C) OP_CONNECT_ONLY=1 ;;
    S) OP_SHUTDOWN=1 ;;
    n)
      OP_NAMESPACE=1
