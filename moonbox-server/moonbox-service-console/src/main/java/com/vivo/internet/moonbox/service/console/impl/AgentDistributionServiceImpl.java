...
import com.vivo.internet.moonbox.service.console.netty.BootNettyChannelInboundHandlerAdapter;
...
public class AgentDistributionServiceImpl implements AgentDistributionService {
     ...
     * @throws Exception 异常信息
     */
    private String startServerAgent(TaskRunInfoWithBLOBs taskRunInfo, String sandboxDownLoadUrl, String moonboxDownLoadUrl, String agentConfig,MoonBoxLogService moonBoxLogService) throws Exception {
       // Machine machine = JSON.parseObject(taskRunInfo.getRunHosts(), Machine.class);
        ...
        /**
         * 使用netty-server端调netty-client端，完成命令执行
         */
        String appName = taskRunInfo.getAppName();
        ...
        List<ActiveAppServerHostInfoVo> activeAppServerHostInfoVos1 = appServerService.selectAppServerHostInfoListFromOnlineState(activeAppServerHostInfoVo);
        String channelId;
        if (activeAppServerHostInfoVos1.size()>0){
            //全部执行
            //TODO:选择执行
            for (ActiveAppServerHostInfoVo appServerHostInfoVo : activeAppServerHostInfoVos1) {
                channelId = appServerHostInfoVo.getChannelId();
                io.netty.channel.Channel channel = bootNettyChannelInboundHandlerAdapter.getChannel(channelId);
                ByteBuf byteBuf = Unpooled.copiedBuffer(command+"\n", CharsetUtil.UTF_8);
                channel.writeAndFlush(byteBuf);
                log.info("启动sandbox,moonbox,channelId:{}", channel);
            }
        }else {
            return appName+"服务不在线";
        }


        //TODO 启动执行成功判断
        return "sandbox-agent、moonbox-agent启动完成";
